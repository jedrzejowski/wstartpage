...
{
  pub async fn get_one(&self, name: &str) -> Result<T, DataSourceError> {
    ...
    let mut json: serde_yml::Mapping = match serde_yml::from_str(file.as_str()) {
      Ok(json) => json,
    ...
    let entity = match serde_yml::from_value(serde_yml::Value::Mapping(json)) {
      Ok(entity) => entity,
      Err(err) => return Err(DataSourceError::Anyhow(err.into())),
    };
  ...
  pub async fn get_all_names(&self) -> Result<Vec<String>, DataSourceError> {
  ...
  pub async fn update_one(&self, name: &str, obj: &T) -> Result<(), DataSourceError> {
    let yaml = serde_yml::to_string(obj)
    ...
    match fs::write(yaml_path, yaml).await{
      Ok(_) => {}
      Err(err) => {
        return Err(DataSourceError::Anyhow(err.into()));
      }
    }
