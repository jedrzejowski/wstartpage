...
const CenterJS: FC<Options> = React.memo(props => {
  ...
  // odświeżanie
  useFontFaceObserver(props.fontFamily ? [{family: props.fontFamily}] : []);

  return <canvas
    ref={(element) => centerJS(element, props)}
    height={props.height}
    width={props.width}
    style={{display: 'block'}}
  />;
...
function centerJS(canvas: HTMLCanvasElement | null, options: Options) {
   ...
   */
  function measureOffsets(text: string, fontFamily: string, fontSize: number) {
    /**
